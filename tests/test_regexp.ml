...
module Regexp = struct
    ...
    | { Loc.txt = Code s1; loc = loc1 } :: { Loc.txt = Code s2; loc = loc2 } :: es ->
      let e12 =
        Loc.
          {
            txt = Code (s1 ^ s2);
            loc = { loc_start = loc1.loc_start; loc_end = loc2.loc_end; loc_ghost = loc1.loc_ghost || loc2.loc_ghost };
          }
      in
      collect_code (e12 :: es)
    | es -> es
  ...
  and simplify' = function
    ...
    | Seq es ->
      ...
      in
      (match es with [ e ] -> e.Loc.txt | es -> Seq es)
    | Alt es ->
      let es = es |> List.map simplify |> List.map (function { Loc.txt = Alt es; _ } -> es | e -> [ e ]) |> List.flatten in
      (match es with [ e ] -> e.Loc.txt | es -> Alt es)
  ...
  let rec equal' e1 e2 =
    ...
    | Seq es1, Seq es2 | Alt es1, Alt es2 -> (try List.for_all2 equal' es1 es2 with Invalid_argument _ -> false)
  ...
  let rec pp_debug ppf self =
    ...
    let pp_loc ppf loc =
      ...
      if loc <> none then begin
        if loc.loc_start.pos_lnum = loc.loc_end.pos_lnum then
          Format.fprintf ppf "@%a-%d" pp_pos loc.loc_start (loc.loc_end.pos_cnum - loc.loc_end.pos_bol)
        else Format.fprintf ppf "@%a-%a" pp_pos loc.loc_start pp_pos loc.loc_end
      end
    in
    (match self.txt with
    | Code s -> fprintf ppf "(Code %S)" s
    | Seq es ->
      fprintf ppf "(Seq ";
    ...
    | Alt es ->
      ...
      List.iter (pp_debug ppf) es;
      fprintf ppf ")"
    | Opt e -> fprintf ppf "(Opt %a)" pp_debug e
...
let gen_name =
  ...
  let idrletter i =
    if i = 0 then '_'
      ...
      let i = i - 1 in
      if i < 26 then Char.chr (0x61 + i)
      else (
        let i = i - 26 in
        if i < 26 then Char.chr (0x41 + i)
        else (
          let i = i - 26 in
          assert (i < 10);
          Char.chr (0x30 + i))))
...
let gen_regexp =
  ...
  @@ fix
  @@ fun self n ->
  let gen_seq = map (fun es -> mknoloc (Seq es)) (0 -- 10 >>= fun k -> list_size (return k) (self (n / max 1 k))) in
  let gen_alt = map (fun es -> mknoloc (Alt es)) (2 -- 10 >>= fun k -> list_size (return k) (self (n / max 1 k))) in
...
let shrink_regexp =
  ...
  let rec shrink e =
    ...
    | Alt (e :: es) -> map2 (fun e es -> mknoloc (Alt (e :: es))) (shrink e) (list ~shrink es)
...
let arb_regexp = Q.make ~print:Regexp.show_debug ~shrink:shrink_regexp gen_regexp
...
let test_parse s =
  ...
  | Ok (e, Ok _), Error `Parse_error ->
    if Regexp.has_anon_capture e then true
    else Q.Test.fail_reportf "Parsed to %a and converted to Re.t, but should be invalid" Regexp.pp_debug e
  | Error err, Ok _ -> Q.Test.fail_reportf "Failed to parse valid %s: %a" s pp_location_error err
  | Ok (e, Error _), Ok _ -> Q.Test.fail_reportf "Parsed to %a but conversion to Re.t failed" Regexp.pp_debug e
  | Ok (_, Ok _), Ok _ ->
